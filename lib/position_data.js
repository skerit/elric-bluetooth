 ...
 * @version  0.1.0
 */
var PData = Function.inherits('Informer', function PositionData(position_calculator) {
...
PData.setMethod(function setData(locator_id, value, date) {
	...
	if (this._seen_locators.indexOf(locator_id) > -1) {
		...
		for (i = 0; i < this.position_calculator.locator_ids.length; i++) {
			...
			// Remove a sample count when there are more then 5 elements
			if (this.sample_counts[id].length > 5) {
...
PData.setMethod(function getNormalizedInput() {
	...
	for (id in this.values) {
		...
		if (this.sample_counts[id]) {
			freq = Math.sum(this.sample_counts[id]) / this.sample_counts[id].length;
		} else {
			freq = 0;
		}
